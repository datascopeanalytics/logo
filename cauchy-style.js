...
var GenerativePoly = function(text, passedColor) {
    ...
    var dtheta, sigma=1;
    for(var i=0; i<8; i++) {
        dtheta = censored_cauchy(2*Math.PI/9, sigma, r, theta, bounds);
        theta += dtheta;
...
function random_allowed_in_region(bounds, outerBounds, theta) {
    ...
    // intersect the line with the acceptable region
    console.log('before', acceptable_region)
    var segment = line.intersect(acceptable_region);
    console.log('boom', segment.bounds)
    segment.strokeColor = "red";

    // // cut line on inner bounds
    // console.log('boy', line.bounds)
    // var newline = bounds.subtract(line);
    // newline.strokeColor = 'red';
    //
    // // cut line on outer bounds
    // console.log('girl', newline.bounds)
    // //line = outerBounds.intersect(line);

    // sample uniformly at random on that slice

}

function censored_cauchy () {
